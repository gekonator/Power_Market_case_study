...
output_path = (
    project_root
    / "outputs/figures/intraday_decision_snapshot_1205_16_19.png"
)

products = ["16:00:00", "17:00:00", "18:00:00"]
publication_time = pd.to_datetime("12:04:00", format="%H:%M:%S")
decision_time = pd.to_datetime("12:05:00", format="%H:%M:%S")
chart_start = pd.to_datetime("08:00:00", format="%H:%M:%S")
chart_end = pd.to_datetime("12:10:00", format="%H:%M:%S")
...
# The chart is a decision snapshot: only trades strictly before 12:05
# are included. The first post-decision window is used separately for entry.
trades = trades[
...
for ax, delivery_start in zip(axes, products):
    ...
    ax.axvline(
        ...
        color="tab:green",
        linestyle="--",
        label="Decision time and cutoff, 12:05 UTC",
    )
    ax.set_title(
...
fig.suptitle(
    "Ex-ante intraday price and liquidity snapshot at 12:05 UTC",
    fontsize=14,
    y=1.0,
)
fig.tight_layout()
